...
## 🟢 v1.6.1: 官方金流升級與介面防錯 (Official Payment & Layout Sync)
**發布重點**：正式將系統底層金流資料庫與前台推播訊息，全面更換為官方最新的匯款帳號，並將介面防錯機制再度升級。
* **臺灣銀行金流切換**：全面停用並將 `contractTerms.ts` 與 LINE 後端推播 API (`notify-contract.ts`)，同步更新為指定的 `004 (臺灣銀行)` 帳號 `007001004263`。
* **戶名核對防錯渲染**：為解決大額轉帳時必須對接戶名的痛點，在合約頁面的付款區塊中獨立渲染出「收款人戶名：貳拾伍數據顧問企業社廖天佑」，讓客戶匯款前能直接看到完整全名，大幅度降低匯錯風險。
* **推播同步一致性**：上述最新的帶入參合約連結與新版臺灣銀行帳戶，均已 100% 同步整合至 Cloudflare 生成的 LINE 通知中，客戶在 LINE 內也可以輕鬆無腦複製帳號。
