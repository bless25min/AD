    ...
    <div className={`${containerBaseClass} ${stickyClass}`} style={isSticky ? { pointerEvents: 'none' } : {}}>
      ...
      >
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          >
            {/* LINE icon */}
            <svg viewBox="0 0 24 24" className="w-6 h-6 mr-3 flex-shrink-0 fill-current" xmlns="http://www.w3.org/2000/svg">
            ...
            <span className="truncate">先加 LINE，拿你的下一步診斷</span>
          </button>
          
          {/* 次按鈕 (Phone) */}
          <a 
            href="tel:0900000000"
            className="w-full relative inline-flex items-center justify-center px-6 py-4 text-lg font-bold text-slate-800 transition-all duration-300 bg-white border border-slate-200 rounded-2xl hover:bg-slate-100 shadow-[0_4px_10px_rgba(255,255,255,0.1)] hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-white/50 text-center"
          >
            <Phone className="w-5 h-5 mr-3 text-slate-700" />
            <span className="truncate">直接打給我，15 分鐘先拆問題</span>
          </a>
